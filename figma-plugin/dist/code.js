...
function exportNode(node, frame, warnings) {
    ...
    if (node.type === 'TEXT') {
        if (!hasRepresentableTypography(node, warnings))
            return null;
        if (!hasRepresentableTextPaint(node, warnings))
            return null;
...
function ancestorClipBounds(node, initial) {
    let clip = initial;
    let parent = node.parent;
...
function textLabel(node, fallback) {
    ...
    if (hasPluginData(node, 'bound_text_value') && boundTextLabelFromSuffix(node.characters, storedValue) === null)
        return null;
...
function boundTextLabel(node, fallback, currentValue) {
    ...
    if (edited !== null)
        return edited;
    if (hasPluginData(node, 'bound_text_label'))
        return stored;
...
function hasPluginData(node, key) {
...
function hasCanonicalMetricCardStyle(node) {
    if (node.type !== 'FRAME')
        return false;
...
function hasRepresentableTextPaint(node, warnings) {
...
function textWeight(node) {
    if (typeof node.fontName !== 'object')
        return undefined;
    const style = node.fontName.style.toLowerCase().replace(/[\s-]+/g, '');
    if (/thin|hairline/.test(style))
        return 100;
    if (/extralight|ultralight/.test(style))
        return 200;
    if (/light/.test(style))
        return 300;
    if (/medium/.test(style))
        return 500;
    if (/semibold|demibold/.test(style))
        return 600;
    if (/extrabold|ultrabold/.test(style))
        return 800;
    if (/black|heavy/.test(style))
        return 900;
    if (/bold/.test(style))
        return 700;
    return 400;
