...
Forecast presentation options are opt-in: setting `uvX` adds a `UV n` column from each row's `uv_index` (or `uvIndex`), and `alternateColumnColor` alternates text color across the columns that are actually visible. Omitting either preserves legacy output. `sampleRenderData()` includes representative UV values so the editor preview exercises the UV column without Home Assistant.
