...
`text` and `metric` items can interpolate entity source keys with `{{ key }}` and safe formatter filters such as `{{ minutesAsleep | minutes }}`. The `minutes` preset converts an integer minute value such as `417` to `6h 57m`; `time` accepts timezone-qualified RFC 3339 timestamps, while `date` also accepts strict `YYYY-MM-DD` calendar dates without UTC day shifting. Invalid or ambiguous date/time inputs remain raw. Optional `data.selectors` entries choose a sanitized state/attribute path for a source, for example `weatherTemperature: attributes.forecast.0.temperature`. Figma bridge saves accept only paths eligible for disclosure by the entity bridge, bounded to the exported attribute depth and first eight array entries. Layouts without selectors continue to use the entity state exactly as before. Text items wrap at approximate character boundaries and are clipped to their configured width and height. `/screen.svg`, `/screen.png`, `/render`, and `/preview` all use the same renderer in `trmnl-ha-layout/src/render.ts`. `/screen.*?sample=1` and `/render?sample=1` use sample data instead of live Home Assistant data.
...
4. Click **Create 800x480 TRMNL Frame** to load the backend frame style and create a matching 800x480 e-ink frame.
...
7. Choose **Format**: `Raw`, `Minutes → hours/minutes`, `Time`, or `Date`. Time accepts timezone-qualified RFC 3339 timestamps; Date also accepts strict `YYYY-MM-DD` values. For `sensor.google_health_sleep_latest_minutes_asleep`, select `State` plus `Minutes → hours/minutes` to preview and save `417` as `6h 57m`.
...
- Only visible text nodes and plugin-created metric card frames matching the backend frame colors and renderer geometry export cleanly. Hidden content and the generated guide label are omitted; extra visible card children, justified text, unsupported bound nodes, non-normal node/ancestor/paint blend modes, text strokes/effects, over-limit labels/static text, and content clipped by ancestors produce warnings and are skipped.
...
The Figma plugin exports metric cards only when their container and bound label/value nodes still match the supported renderer template, including adaptive value/label placement for short cards and each part's canonical role-specific paint. Moving or resizing the card and editing its label or value binding are supported when the child geometry still matches the renderer; visual, opacity, blend-mode, child-position, and typography changes are skipped with an export warning because the saved layout cannot represent them. Export requests are correlated so preview and save results cannot be confused.
